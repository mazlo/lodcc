...
do_convert()
{
    ...
    if [[ -d "$FPATH_STRIPPED" ]]; then
        ...
        for f in `find "$FPATH_STRIPPED" -type f \( ! -name "*.bib" ! -name "*.csv" ! -name "*.log" ! -name "*.py" ! -name "*.pl" ! -name "*.sh" ! -name "*.tsv" ! -name "*.txt" ! -name "*.md" ! -name "*.tab" ! -name "*.xls" ! -name "*.xlsx" ! -name "*.xsl" ! -name "LICENSE" ! -name "log" ! -name "README" ! -name "Readme" ! -name "readme" \) `; do
